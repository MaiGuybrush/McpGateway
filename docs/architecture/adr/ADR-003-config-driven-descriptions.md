...
| 缺點 | 緩解措施 |
|------|----------|
| 描述文字與程式碼耦合 | 描述文字可透過資源檔（`.resx`）國際化，將文案抽離而不失去編譯期檢查 |
| 非技術人員無法編輯描述 | 若此需求成真，即為升級選項 C（混合方案）的觸發條件之一；亦可提供基於 XML 註解的描述生成器輔助 |

### 預期影響（估計值，待實作後驗證）

> 以下為 2026-07-31 決策時的估計，**尚無實測數據支撐**，應於 MVP 完成後回頭驗證。

- Tool 開發速度提升 ~30%（免去設定檔同步作業）
- 啟動失敗風險降低 ~80%（無設定檔脫鉤導致的啟動阻塞）
- 維護成本降低 ~50%（單一維護點）
...
- **[ADR-004](ADR-004-startup-validation.md)**：驗證機制（B 無需驗證，C 需測試覆寫邏輯）
- **[ADR-006](ADR-006-security-model.md)**：設定檔結構（C 需 `descriptionOverrides`）
- **[ADR-005](ADR-005-tool-versioning.md)**：版本控制（描述變更是否算破壞性變更？）
- **[ADR-009](ADR-009-department-gateway-split.md)**：多部門拓撲。見下方「多部門環境下的影響」

### 🔄 【ADR-009】多部門環境下的影響（2026-08-02）

本 ADR 的核心決策（選項 B 程式碼內聯描述）在多部門拓撲下**完全不變** ——
`[Description]` 標註隨工具類別留在**部門專案**內，`McpGateway.Core` 負責掃描與套用。
這正是 [ADR-009 D2 極薄部門專案](ADR-009-department-gateway-split.md) 的理想切分：
描述文字屬「內容」歸部門，掃描機制屬「框架」歸 Core。

兩處需釐清：

**1. 升級到選項 C 的觸發條件「Tool 數量 > 20」，計數範圍為何？**

→ 採 **單一部門計數**。設定檔管理的痛感發生在「我這個部門的工具改描述很麻煩」，
與其他部門無關。各部門可獨立決定是否啟用覆寫機制。

**2. 若升級到選項 C，覆寫設定檔放哪？**

→ 每個部門專案各自一份（`McpGateway.{Dept}/appsettings.json` 的
`McpGateway:DescriptionOverrides` 區段），**合併邏輯實作於 Core**。

遷移成本重估：

| 項目 | 原估（單一服務） | 多部門修正 |
|------|----------------|-----------|
| Core 合併邏輯實作 | 3 人天 | 3 人天（**不變** —— 只實作一次） |
| 各部門設定檔導入 | — | 每部門約 0.5 人天，且**可選、可不做** |

結論：B→C 遷移成本**未因多部門而顯著增加**，因為邏輯集中於 Core。
